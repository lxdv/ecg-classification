...
from models.models_1d import HeartNet
...
class Trainer1D(BaseTrainer):
    ...
    def _init_dataloaders(self):
        train_loader = EcgDataset1D(self.train_json, self.mapping_json).get_dataloader(
            batch_size=self.config['batch_size'], num_workers=self.config['num_workers']
        )
        val_loader = EcgDataset1D(self.val_json, self.mapping_json).get_dataloader(
            batch_size=self.config['batch_size'], num_workers=self.config['num_workers']
        )
