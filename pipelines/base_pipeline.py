...
class BasePipeline:
    ...
    def run_pipeline(self):
        ...
        for label, peak in zip(pd_class, pd_peaks):
            if peak < len(self.pipeline_loader.dataset.signal) and self.mapper[label] != 'N':
                annotations.append({
                    "x": peak,
        ...
        fig = go.Figure(data=go.Scatter(x=list(range(len(self.pipeline_loader.dataset.signal))), y=self.pipeline_loader.dataset.signal))
        ...
        fig.write_html(osp.join(self.res_dir, osp.basename(self.config['ecg_data'] + '.html')))
